...
function parseStreams(data) {
  return __async(this, null, function* () {
    ...
    const validItems = data.streams.filter((item) => {
      const cleanedTitle = cleanText(item?.title);
      if (!cleanedTitle.toLowerCase().includes("1080")) return false;
...
function getStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    ...
    try {
      ...
      if (!isSeries) {
        return yield fetchStreams(`${SOLUNIX_API}/stream/movie/${imdbId}.json`);
      }

      return yield fetchFirstValid([
        `${SOLUNIX_API}/stream/series/${imdbId}:${pad2(s)}:${pad2(e)}.json`,
        `${SOLUNIX_API}/stream/series/${imdbId}:${parseInt(s, 10) || 1}:${parseInt(e, 10) || 1}.json`,
      ]);
    } catch {
      return [];
    }
